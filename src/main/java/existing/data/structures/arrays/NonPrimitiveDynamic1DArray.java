...
/**
 * Dynamic generic one-dimensional array wrapper.
 * Stores values in a reflection-created backing array.
 * @param <T> type of element stored in the array
 */
public class NonPrimitiveDynamic1DArray<T> {

  int size;
  T[] array;

  /**
   * Creates a dynamic generic array using the given type and size.
   * @param className runtime type of the elements
   * @param size starting length of the array
   */
  public NonPrimitiveDynamic1DArray(Class<T> className, int size) {
    this.size = size;
  ...
  public void add(int index, T element) {
    if (index < size) {
      ...
      // Resize first when the requested index is outside the current capacity.
      array = resizer(true, index);
      add(index, element);
    }
  }
   ...
   * @param index position to clear from the array
   */
  public void remove(int index) {
    if (index < size) {
      array = resizer(false, index);
    } else
      throw new ArrayIndexOutOfBoundsException(
          "The Index passed by you is beyond the size of the array");
  }
  ...
  /**
   * Builds a resized array for add or remove operations.
   * @param isIncreasing true when growing, false when shrinking
   * @param index index involved in the resize operation
   * @return resized array reference used by the class
   */
  public T[] resizer(Boolean isIncreasing, int index) {
    ...
    if (isIncreasing) {
      ...
      temp = copyExistingElements(isIncreasing, temp);
      return temp;
    } else {

      int nullCounter = 0;

      // Count removable null slots from the selected index to the end.
      ...
      @SuppressWarnings("unchecked")
      T[] temp = (T[]) Array.newInstance(array.getClass(), size - nullCounter);
    }
    return this.array;
  }

  /**
   * Copies existing elements into the provided target array.
   * @param isIncreasing true when expanding, false when shrinking
   * @param temp array that receives the copied elements
   * @return target array after copying values
   */
  private T[] copyExistingElements(Boolean isIncreasing, T[] temp) {

    // Growth keeps every current element; shrinking copies only what fits.
    if (isIncreasing) {
